...
def build_signal_panels(
    ...
    release_by_report = {
        pd.Timestamp(report_date): pd.Timestamp(release_date)
        for report_date, release_date in release_dates.items()
    }
    outputs: dict[float, pd.DataFrame] = {}
    for threshold in (0.5, 1.0):
        panel = pd.DataFrame(0.0, index=daily_index, columns=sorted(CFTC_CODES))
        for report_date, score in scores.iterrows():
            release_date = release_by_report[pd.Timestamp(report_date)]

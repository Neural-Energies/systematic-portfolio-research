    ...
    for timeframe in TIMEFRAMES:
        ...
        for training_sessions in (20, 40):
            ...
            for threshold in (0.75, 1.25):
                name = (
                    f"h019_{timeframe:03d}m_train{training_sessions:02d}_"
                    f"threshold{str(threshold).replace('.', 'p')}"
                )
                variant = PcaVariant(name, timeframe, training_sessions, threshold)
                variants.append(variant)
                signal_weights = _balanced_tail_weights(scores, threshold)
                ...
                daily_gross = (
                    gross_by_product.groupby(date_by_bar).sum().reindex(daily_index, fill_value=0.0)
                )
                daily_sides = (
                    sides_by_product.groupby(date_by_bar).sum().reindex(daily_index, fill_value=0.0)
                )
...
def run_factory(project_root: Path, *, one_way_cost_bps: float = 1.0) -> Path:
    ...
    for variant_name in gross.columns:
        ...
        for symbol in product_gross[variant_name].columns:
            ...
            definition = {"hypothesis_id": "H019", "symbol": symbol, **variant}
    ...
    (master / "MASTER_INDEX.md").write_text(
        "\n".join(
            [
                "# H019 PCA Residual Reversal — Master Archive",
                "",
                f"- Run: `{run_id}`",

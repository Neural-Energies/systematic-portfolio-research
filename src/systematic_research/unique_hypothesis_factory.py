...
def _daily_compound(
    values: pd.Series, dates: pd.Series, daily_index: pd.DatetimeIndex
) -> pd.Series:
    output = values.add(1.0).groupby(dates).prod().sub(1.0)
    output = output.reindex(daily_index).fillna(0.0)
    output.index = pd.to_datetime(output.index)
    return output.astype(float)
...
def build_unique_population_pass3(
    minute_data: pd.DataFrame,
) -> tuple[list[UniqueStrategySpec], pd.DataFrame, pd.DataFrame]:
    """Build lower-turnover preregistered H012-H016 strategy families."""
    daily_index = pd.DatetimeIndex(sorted(pd.to_datetime(minute_data["trading_date"]).unique()))
    symbols = sorted(minute_data["symbol"].astype(str).unique())
    specs: list[UniqueStrategySpec] = []
    gross: dict[str, pd.Series] = {}
    ...
    session_volume = sessions.pivot(
        index="trading_date", columns="symbol", values="session_volume"
    ).reindex(daily_index)
    session_return = session_close.div(session_open).sub(1.0).fillna(0.0)

    # H012: futures-specific Friday-minus-Monday weekend premium.
    weekday = pd.Series(daily_index.dayofweek, index=daily_index)
    weekend_position = weekday.eq(4).astype(float).sub(weekday.eq(0).astype(float))
    for symbol in symbols:
        _append_variant(
            specs,
            gross,
            ...
            threshold=0.0,
            direction=1,
            gross_return=weekend_position * session_return[symbol],
            trade_sides=weekend_position.abs() * 2.0,
        )

    # H013: weekly return reversal conditional on bounded lagged volume expansion.
    short_volume = session_volume.rolling(5, min_periods=5).mean()
    long_volume = session_volume.rolling(20, min_periods=20).mean().replace(0.0, np.nan)
    lagged_volume_ratio = short_volume.div(long_volume).shift(1)
    for lookback in (3, 5):
        lagged_return = session_return.rolling(lookback, min_periods=lookback).sum().shift(1)
        direction = lagged_return.apply(np.sign).mul(-1.0)
        for threshold in (1.0, 1.25):
            eligible = lagged_volume_ratio.ge(threshold) & lagged_volume_ratio.le(3.0)
            positions = direction.where(eligible, 0.0).fillna(0.0)
            for symbol in symbols:
                position = positions[symbol]
                _append_variant(
                    specs,
                    ...
                    threshold=threshold,
                    direction=-1,
                    gross_return=position * session_return[symbol],
                    trade_sides=position.abs() * 2.0,
                )
    ...
    bars["bar_return"] = bars["close"].div(bars["open"]).sub(1.0)
    local_timestamp = pd.to_datetime(bars["timestamp_utc"], utc=True).dt.tz_convert(
        "America/New_York"
    )
    local_minutes = local_timestamp.dt.hour.mul(60).add(local_timestamp.dt.minute)
    overnight_mask = local_minutes.ge(18 * 60) | local_minutes.lt(9 * 60 + 30)
    core_mask = local_minutes.ge(9 * 60 + 30) & local_minutes.lt(16 * 60)

    # H016 input: realized third moment of causal 15-minute returns by session.
    realized_skew = (
        bars.groupby(["symbol", "trading_date"], sort=True)["bar_return"]
        .skew()
        .unstack("symbol")
        .reindex(daily_index)
    )
    for smoothing in (1, 3, 5):
        smoothed_skew = realized_skew.rolling(smoothing, min_periods=smoothing).mean()
        skew_score = smoothed_skew.rank(axis=1, pct=True).sub(0.5).mul(-2.0)
        for threshold in (0.2, 0.4):
            positions = (
                skew_score.where(skew_score.abs().ge(threshold), 0.0)
                .apply(np.sign)
                .shift(1)
                .fillna(0.0)
            )
    ...
    for symbol in symbols:
        product = bars.loc[bars["symbol"].eq(symbol)].copy()
        dates = pd.to_datetime(product["trading_date"])
        product_overnight = overnight_mask.loc[product.index]
        product_core = core_mask.loc[product.index]
        overnight_return = _daily_compound(
            product.loc[product_overnight, "bar_return"],
            dates.loc[product_overnight],
            daily_index,
        )
        core_return = _daily_compound(
            product.loc[product_core, "bar_return"],
            dates.loc[product_core],
            daily_index,
        )

        # H014: fade unusually large overnight information moves during core hours.
        overnight_score = _rolling_series_zscore(overnight_return, history=80)
        for threshold in (0.0, 0.5, 1.0):
            position = (
                overnight_score.where(overnight_score.abs().ge(threshold), 0.0)
                .apply(np.sign)
                .mul(-1.0)
                .fillna(0.0)
            )
            _append_variant(
                ...
                threshold=threshold,
                direction=-1,
                gross_return=position * core_return,
                trade_sides=position.abs() * 2.0,
            )

        # H015: variance-ratio sign chooses continuation versus reversal.
        returns = session_return[symbol]
        for history in (40, 80):
            one_period_variance = (
                returns.rolling(history, min_periods=history).var().replace(0.0, np.nan)
            )
            for horizon in (2, 5):
                horizon_return = returns.rolling(horizon, min_periods=horizon).sum()
                variance_ratio = (
                    horizon_return.rolling(history, min_periods=history)
                    .var()
                    .div(one_period_variance.mul(float(horizon)))
                    .sub(1.0)
                )
                recent_direction = horizon_return.apply(np.sign)
                for threshold in (0.10, 0.25):
                    regime_direction = variance_ratio.where(
                        variance_ratio.abs().ge(threshold), 0.0
                    ).apply(np.sign)
                    position = recent_direction.mul(regime_direction).shift(1).fillna(0.0)
                    _append_variant(
                        specs,
                        gross,
                        sides,
...
def walk_forward_select(
    ...
    *,
    ...
    minimum_training_score: float | None = 0.0,
    ...
    for evaluation_fold in range(1, folds):
        ...
        for hypothesis in sleeve_oos.columns:
            ...
            for name in names:
                ...
                if minimum_training_score is not None and score <= minimum_training_score:
                    continue
...
def run_factory(
    ...
    elif research_pass == 3:
        hypothesis_ids = ["H012", "H013", "H014", "H015", "H016"]
        specs, gross, sides = build_unique_population_pass3(minute_data)
    else:
        raise ValueError("research_pass must be 1, 2, or 3")
    net = gross - sides * one_way_cost_bps / 10_000.0
    minimum_training_score = None if research_pass == 1 else 0.0
    sleeves, portfolio, selections = walk_forward_select(
        specs, net, sides, minimum_training_score=minimum_training_score
    )
...
def main() -> None:
    ...
    parser.add_argument("--research-pass", type=int, choices=(1, 2, 3), default=3)

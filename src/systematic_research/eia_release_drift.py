...
def chained_product_selection(
    ...
    for evaluation_number in range(1, 4):
        ...
        if selected:
            evaluation_frame = cast(
                pd.DataFrame, net.reindex(columns=selected).iloc[evaluation_rows]
            )
            portfolio.iloc[evaluation_rows] = evaluation_frame.sum(axis=1)
...
def replay(net: pd.DataFrame, selections: pd.DataFrame) -> pd.Series:
    ...
    for fold in range(2, 5):
        names = (
            selections.loc[selections["evaluation_fold"].eq(fold), "strategy"].astype(str).tolist()
        )
        if names:
            rows = blocks[fold - 1]
            evaluation_frame = cast(pd.DataFrame, net.reindex(columns=names).iloc[rows])
            output.iloc[rows] = evaluation_frame.sum(axis=1)
